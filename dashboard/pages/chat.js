...
async function sendChatMessage() {
  ...
  // Auto-detect build/create intent — use /api/chat which auto-saves files
  var buildKW = ['направи', 'създай', 'изгради', 'build', 'create', 'generate', 'напиши', 'landing', 'html', 'сайт', 'лендинг', 'проект', 'page', 'code'];
  var buildIntent = project && !skill && buildKW.some(function(k) { return message.toLowerCase().indexOf(k) !== -1; });
  
  if (buildIntent) {
    input.value = '';
    input.style.height = 'auto';
    addChatMessage('user', '🤖 ' + message, agent);
    var resultMsg = addChatMessage('assistant', '', agent, true);
    var resultEl = resultMsg ? resultMsg.element.querySelector('.chat-message-content') : null;
    try {
      var resp = await fetch('/api/chat', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({agent: agent, message: message, project: project})
      });
      var data = await resp.json();
      var respData = data.response || data;
      var code = respData.content || '';
      var files = respData.files_created || [];
      if (resultEl) {
        resultEl.textContent = code.slice(0, 3000);
        if (files.length > 0) {
          resultEl.textContent += '\n\n✅ ' + files.length + ' files saved to ' + project + '/';
          files.forEach(function(f) { resultEl.textContent += '\n📁 ' + f; });
        }
      }
    } catch (err) {
